"""Visual-ablation grounding audit for SurgMotionVQA checkpoints.

Read-only diagnostic: does NOT retrain or modify any checkpoint. For a
stratified subsample of visually-grounded questions (tool_presence,
forceps_type, tissue_cutting) on visual_status=="valid" segments from a
model's own held-out validation split, this generates two predictions per
item with the exact same checkpoint/weights:

  - "real":  the true decoded clip and its real frame_valid_mask.
  - "blank": frame_valid_mask forced to all-invalid, which routes
             SurgMotionVQA.encode_visual through its learned
             no_visual_prefix path and never calls the SurgMotion encoder.

If a checkpoint is actually grounding these answers in the endoscope feed,
"real" should score higher than "blank" and predictions should change often.
If "real" and "blank" are nearly identical, the checkpoint is answering from
language prior / question-text pattern alone, independent of what generalization
gain (if any) shows up in aggregate BERTScore.
"""
...
from predict_surgmotion_vqa import answer_tensor, assert_gpu_idle, load_model
from train_surgmotion_vqa import SurgMotionQADataset
...
def main():
    ...
    parser.add_argument(
        "--question-types",
        nargs="+",
        default=["tool_presence", "forceps_type", "tissue_cutting"],
    )
    ...
    model, tokenizer, num_frames = load_model(args.checkpoint)
    ...
    dataset = SurgMotionQADataset(args.vqa_json, args.cache_root, is_train=False, num_frames=num_frames)
    ...
    for position, index in enumerate(indices):
        ...
        real_valid = sample["frame_valid_mask"].unsqueeze(0)
        blank_valid = torch.zeros_like(real_valid)

        prediction_real = answer_tensor(
            model, tokenizer, video, real_valid, item["question"], args.max_new_tokens
        )
        prediction_blank = answer_tensor(
            model, tokenizer, video, blank_valid, item["question"], args.max_new_tokens
        )
        row = {
            "id": item["id"],
            ...
            "prediction_blank": prediction_blank,
            "changed": normalize(prediction_real) != normalize(prediction_blank),
        }
        rows.append(row)
        print(
            f"[{position + 1}/{len(indices)}] {item['id']} "
            f"real='{prediction_real}' blank='{prediction_blank}' changed={row['changed']}"
        )

    real_preds = [row["prediction_real"] for row in rows]
    blank_preds = [row["prediction_blank"] for row in rows]
    ...
    for row, real_score, blank_score in zip(rows, bert_real, bert_blank):
        ...
        row["bertscore_blank"] = blank_score
    ...
    def summarize(subset):
        ...
        return {
            ...
            "bertscore_blank_mean": sum(row["bertscore_blank"] for row in subset) / len(subset),
